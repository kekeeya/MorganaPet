...
**基于Persona 5 的Mac桌面宠物，SwiftUI + AppKit。目前可以监控 Claude 和 Codex 的额度 & 电脑运行的基本情况；根据CPU利用率为速度在状态栏奔跑；并且可以在桌面用游戏内风格渲染日历，显示当前日期、星期、天气和时段。**


<p align="center">
  <img src="docs/calendar.gif" height="200" alt="桌面日历">
  <img src="docs/running.gif" height="100" alt="菜单栏">
...
菜单栏里的Mona会一直跑，**跑多快取决于电脑CPU利用率**。设置里可以反转奔跑速度。
...
「查看本机状态」会报 CPU、内存、硬盘、电池和网络，5 秒刷新一次。



### 桌面日历

菜单栏「显示/隐藏日历」会在桌面上放一张日期贴纸。采用游戏内同样美术风格；
贴纸透明的地方点击会穿透到下层窗口。设置 →「日历」里可以调大小、关掉置顶、选城市。


天气取自 [Open-Meteo](https://open-meteo.com)，每半小时一次。
