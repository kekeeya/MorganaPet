...
> 桌宠**默认不显示**，在状态栏菜单或设置里打开；开过之后就记住你的选择。桌面日历同理，
> 见下一节。
...
<details>
<summary>它是怎么画出来的</summary>

贴纸不是一张图，是按设计自己的**五层**叠出来的：所有白底 → 天气图标 → 所有黑底 →
所有白字 → 星期和时段的字。按元素叠总会牺牲一方——星期牌压在日期上，日期身上就横一道黑；
反过来字母又被吞掉。按层叠两种都不会发生。

每个部件摆在哪不是在 App 里算的，而是查一张预先算好的表：月/日 12×31 组，星期、时段、
天气各自按位数和种类展开，加起来两千出头。摆放的规则（按字的缩放位移、月份十位的字距、
天气挂星期牌、字距兜底）都在离线那侧，翻译成 Swift 只会得到两份互相漂移的实现。

补洞、连通块、精确圆盘闭运算是自己写的（`CalendarRaster.swift`）而没用 CoreImage：
`CIMorphologyMaximum` 边缘是糊的，黑底胖一个像素，底下的白描边就从边上透出一道灰。
